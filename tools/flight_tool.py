...
# For global mode, do not force an India-centric default origin.
...
def find_location_mentions(query: str):
    ...
    # Phrase-based location detection using resolve_location_to_iata
    ...
    for length in range(max_phrase_length, 0, -1):
        for i in range(len(tokens) - length + 1):
            ...
            if phrase in {
                "trip", "trips", "travel", "plan", "planning", "worldwide",
                "global", "budget", "hotels", "days", "day", "with", "etc",
                "around", "go", "from", "to", "in", "on", "and"
            }:
                continue
            if resolve_location_to_iata(phrase):
...
def search_flights(query: str, limit: int = 10, from_location: str | None = None, country: str | None = None):
    if not API_KEY:
        return (
    ...
    if not dep_iata and from_location:
        dep_iata = resolve_location_to_iata(from_location)

    if not arr_iata and country:
        country_clean = country.strip()
        if country_clean and country_clean.lower() != "global":
            arr_iata = resolve_location_to_iata(country_clean)
    ...
    if not flight_data:
        # Build a helpful fallback when live data is missing.
        route_text = ""
        ...
        # If we detected a destination airport, suggest common origin airports and carriers.
        suggestions = []
        if arr_iata:
            # Human-friendly names
            arr_airport = AIRPORTS.get(arr_iata, {})
            arr_city = arr_airport.get("city") or arr_airport.get("name") or arr_iata

            # Use the default origin if available, otherwise suggest major nearby hubs
            origins = []
            if DEFAULT_ORIGIN_IATA:
                origins.append(DEFAULT_ORIGIN_IATA)

            # Add country main airport for the origin suggestions if available
            try:
                # Find country code from airport data if possible
                airport_country = str(arr_airport.get("country", "")).upper().strip()
                main_origin = COUNTRY_MAIN_AIRPORT.get(airport_country)
                if main_origin and main_origin not in origins:
                    origins.append(main_origin)
            except Exception:
                pass

            # Common carriers heuristic (country-specific fallback)
            common_carriers = []
            if airport_country == "IN" or (arr_city and "india" in str(arr_city).lower()):
                common_carriers = ["IndiGo", "Air India", "Vistara", "SpiceJet", "Go First"]
            else:
                common_carriers = ["Sample carriers: check major airlines serving the region"]

            suggestions.append(f"No live flight data found{route_text}.")
            suggestions.append(f"Destination detected: {arr_city} ({arr_iata}).")

            if origins:
                suggestions.append("Common origin airports to check: " + ", ".join(origins))

            suggestions.append("Frequent carriers on these routes: " + ", ".join(common_carriers))
            suggestions.append("Note: AviationStack provides live/status flight data, not ticket prices. For fares, use a pricing API such as Amadeus or Skyscanner.")
